...
class CmdMessenger(object):
    ...
    **Note:** the file-like object should be open, and its read method
    ...
    def __init__(self, flobject, fld_separator=",", cmd_separator=";", esc_character="/", readmeth=None):
        ...
        self._file.flushInput() # flush pending data
        self._file.flushOutput() # flush pending data
    ...
    def send_cmd(self, msgid, *args, **kwargs):
        ...
        """
        self._file.write(bytes(str(msgid).encode(self._encoding)))
        for a in args:
            self._file.write(self._fld_sep.encode(self._encoding))
            if type(a) == bool:
                a = int(a)
            self._file.write(str(a).encode(self._encoding))
        self._file.write(self._cmd_sep.encode(self._encoding))
